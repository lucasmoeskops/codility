...
there are many leaves on the river, and the frog can jump between the leaves, but only
...
side of the river (from position −1 to position N). The frog can jump between positions
−1 and N (the banks of the river) and every position containing a leaf.
...
def solution(A):
    ...
    for n in fibonacci:
        ...
        if n < positions and A[n - 1] == 1:
            stack.push((n - 1, 1))
            best[n] = 1

    while stack:
        new_stack = Stack(positions)
        ...
        while stack:
            ...
            for n in fibonacci:
                ...
                if m < len(A) and A[m] == 1 and not best[m]:
                    best[m] = 1
                    new_stack.push((m, jumps + 1))
