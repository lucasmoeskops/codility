...
impact factor of nucleotides contained in the DNA sequence between positions P[K] and
Q[K] (inclusive).
...
that, given a non-empty string S consisting of N characters and two non-empty arrays P
and Q consisting of M integers, returns an array consisting of M integers specifying the
...
def solution(S, P, Q):
    ...
    for letter in S:
        for letter_2 in 'ACGT':
            if letter == letter_2:
                distances[letter_2] = 0
            elif distances[letter_2] >= 0:
                distances[letter_2] += 1

        distance_lookup.append(distances.copy())

    for start, end in zip(P, Q):
        distance = end - start
        lookup = distance_lookup[end]

        for value, letter in enumerate('ACGT', start=1):
            if 0 <= lookup[letter] <= distance:
