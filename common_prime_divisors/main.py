...
def solution(A, B):
    count = 0
    # consider each pair as list
    for t in map(list, zip(A, B)):
        # what are the shared factors
        shared = gcd(*t)
        # check both numbers
        for i in range(2):
            # reduce the number by shared factors until it is 1 or cant
            # be reduced anymore
            sub = shared
            while sub > 1 < t[i]:
                t[i] //= sub
                sub = gcd(t[i], shared)
        # it is valid if the shared factors divide both numbers
        count += all(shared % o == 0 for o in t)
    return count
