...
You are given two non-empty arrays P and Q, each consisting of M integers. These arrays
...
returns an array consisting of M elements specifying the consecutive answers to all the
queries.
...
from codility.lesson11.factorization import array_F

__author__ = 'Lucas Moeskops'


def solution(N, P, Q):
    # We can calculate semi primes as having exactly 2 factors
    F = array_F(N)
    semi_primes = [n for n, f in enumerate(F) if f and F[n // f] == 0]
    direct_lookup = [0] * (N + 1)
    i = 0
    j = 0
    ...
    return [direct_lookup[q] - direct_lookup[p - 1] for p, q in zip(P, Q)]
