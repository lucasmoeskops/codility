...
the first place. For example, the rotation of array A = [3, 8, 9, 7, 6] is
[6, 3, 8, 9, 7] (elements are shifted right by one index and 6 is moved to the first
place).
...
def solution(A, K):
    return list(islice(islice(cycle(A), len(A) - K % (len(A) or 1), None), len(A)))
