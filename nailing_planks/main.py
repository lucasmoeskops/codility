...
are nailed. In other words, you should find a value J such that all planks will be nailed
...
def solution(A, B, C):
    sorted_ab = sorted(zip(A, B), key=itemgetter(0))
    num_nails = bisect_left(
        BinarySearch(partial(check, sorted_ab, C), len(C) + 1),
        1,
        lo=1,
    )
    return -1 if num_nails == len(C) + 1 else num_nails


def check(sorted_ab, C, i):
    sorted_c = sorted(C[:i])
    j = 0

    for start, end in sorted_ab:
        while sorted_c[j] < start:
            j += 1
